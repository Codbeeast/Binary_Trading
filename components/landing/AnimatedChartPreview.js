...
export default function AnimatedChartPreview() {
  ...
  // KPI data
  const kpis = [
    { label: 'Total Trades', value: '$350,240', icon: DollarSign, color: 'text-brand-orange' },
    { label: 'Active Users', value: '12.3k', icon: Activity, color: 'text-brand-gold' },
  ...
  useEffect(() => {
    ...
    const animate = (timestamp) => {
      ...
      // Update data (simulate live ticks) - Throttled to every 200ms
      if (timestamp - lastUpdate > 200) {
        if (Math.random() > 0.5) { // 50% chance to update every 200ms
      ...
      dataPointsRef.current.forEach((point, i) => {
        ...
        const color = isGreen ? 'rgba(34, 197, 94, 0.8)' : 'rgba(239, 68, 68, 0.8)'; // Standard Green/Red
  ...
  return (
    ...
    >
      {/* Glow effect */}
      <div className="absolute inset-0 bg-gradient-to-br from-brand-orange/20 via-transparent to-brand-gold/20 blur-3xl animate-glow-pulse" />
      ...
      <div className="relative bg-dark-700/50 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
        ...
        <div className="px-6 pb-6 grid grid-cols-3 gap-3">
          {kpis.map((kpi, index) => (
            <motion.div
              ...
              className="bg-dark-800/80 backdrop-blur-sm rounded-xl p-3 border border-white/5 hover:border-brand-orange/30 transition-all group"
            >
        ...
        <div className="absolute top-20 right-6 bg-dark-800/90 backdrop-blur-sm rounded-lg p-3 border border-white/10">
          <div className="flex gap-2">
            <button className="w-8 h-8 rounded bg-white/5 hover:bg-brand-orange/20 transition-colors flex items-center justify-center text-xs text-gray-400 hover:text-brand-orange">
              1m
            </button>
            <button className="w-8 h-8 rounded bg-brand-orange/20 text-brand-orange transition-colors flex items-center justify-center text-xs font-semibold">
              5m
            </button>
            <button className="w-8 h-8 rounded bg-white/5 hover:bg-brand-orange/20 transition-colors flex items-center justify-center text-xs text-gray-400 hover:text-brand-orange">
              15m
